...
class CacheConfig 
{
    ...
    /**
     * This must be a service id to a service implementing Psr\Http\Message\StreamFactoryInterface
     * @default 'httplug.psr17_stream_factory'
     * @param ParamConfigurator|mixed $value
     * @return $this
     */
    ...
    /**
     * @default {"blacklisted_paths":[],"methods":["GET","HEAD"],"cache_listeners":[]}
    */
    public function config(array $value = []): \Symfony\Config\Httplug\Plugins\Cache\ConfigConfig
    {
        if (null === $this->config) {
            $this->_usedProperties['config'] = true;
            $this->config = new \Symfony\Config\Httplug\Plugins\Cache\ConfigConfig($value);
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('config', $value)) {
            $this->_usedProperties['config'] = true;
            $this->config = new \Symfony\Config\Httplug\Plugins\Cache\ConfigConfig($value['config']);
            unset($value['config']);
        }
